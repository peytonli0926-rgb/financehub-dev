...
@Slf4j
@Service
public class RepaymentPlanCYCServiceImpl extends ServiceImpl<RepaymentPlanHYMapper, RepaymentPlanHYEntity>
        implements IRepaymentPlanCYCService {
    ...
    public String dataInit(DataInitDTO params) {
        ...
        while (true) {
            ...
            List<String> contractCodeList = passengerVehicleDataService.selectContractCodeByPage(pageInfo);
            ...
            List<SelectRepaymentFromCYCXTDTO> selectRepaymentFromCYCXTDTOList = passengerVehicleDataService.
                    selectRepaymentFromCYCXTGroupInit(dto);
            log.info("乘用车期初数据同步 数据量：" + selectRepaymentFromCYCXTDTOList.size());
            if (selectRepaymentFromCYCXTDTOList == null || selectRepaymentFromCYCXTDTOList.isEmpty()) {
                continue;
            }

            this.saveRepaymentPlanForCYC(selectRepaymentFromCYCXTDTOList);
            log.info("乘用车期初数据同步 End");

            // 同步回笼数据
            log.info("乘用车回笼数据同步 Start--------------");
            List<SelectReceiveRepaymentDTO> receiveRepaymentDTOList = passengerVehicleDataService.selectReceiveRepayment(dto);
            if (receiveRepaymentDTOList != null && !receiveRepaymentDTOList.isEmpty()) {
                this.saveReceiveRepayment(receiveRepaymentDTOList);
            }
        }
        log.info("乘用车回笼数据同步 End--------------");


        // 乘用车资产转让期初数据同步
        start = 0;
        while (true) {
            com.utfinancing.financehub.etl.passvehat.model.SelectContractCodeByPageDTO pageInfo =
                    new com.utfinancing.financehub.etl.passvehat.model.SelectContractCodeByPageDTO();
            pageInfo.setStart(start);
            pageInfo.setEnd(start + eachQueryNum);
            List<String> contractCodeList = passengerVehicleAssetTransferDataService.selectContractCodeByPage(pageInfo);
            if (contractCodeList == null || contractCodeList.isEmpty()) {
                break;
            } else {
                start = start + eachQueryNum;
            ...
            com.utfinancing.financehub.etl.passvehat.model.SelectRepaymentFromCYCXTInputDTO dto1 =
                    new com.utfinancing.financehub.etl.passvehat.model.SelectRepaymentFromCYCXTInputDTO();
            dto1.setContractCodeList(contractCodeList);
            List<com.utfinancing.financehub.etl.passvehat.model.SelectRepaymentFromCYCXTDTO> selectRepaymentFromCYCXTDTOList1 =
                    passengerVehicleAssetTransferDataService.selectRepaymentFromCYCXTGroupInit(dto1);
            log.info("乘用车资产转让期初数据同步 数据量：" + selectRepaymentFromCYCXTDTOList1.size());
            if (selectRepaymentFromCYCXTDTOList1 == null || selectRepaymentFromCYCXTDTOList1.isEmpty()) {
                continue;
            }

            List<SelectRepaymentFromCYCXTDTO> selectRepaymentFromCYCXTDTOList = BeanUtil.
                    copyToList(selectRepaymentFromCYCXTDTOList1, SelectRepaymentFromCYCXTDTO.class);
            this.saveRepaymentPlanForCYC(selectRepaymentFromCYCXTDTOList);
            log.info("乘用车资产转让期初数据同步 End");

            // 同步回笼数据
            log.info("乘用车资产转让回笼数据同步 Start--------------");
            List<com.utfinancing.financehub.etl.passvehat.model.SelectReceiveRepaymentDTO> receiveRepaymentDTOList1 =
                    passengerVehicleAssetTransferDataService.selectReceiveRepayment(dto1);
            if (receiveRepaymentDTOList1 != null && !receiveRepaymentDTOList1.isEmpty()) {
                List<SelectReceiveRepaymentDTO> receiveRepaymentDTOList = BeanUtil.copyToList(
                        receiveRepaymentDTOList1, SelectReceiveRepaymentDTO.class);
                this.saveReceiveRepayment(receiveRepaymentDTOList);
            }
        }
        log.info("乘用车资产转让回笼数据同步 End--------------");
    ...
    public String receivedAmountSync(DataInitDTO params) {
        ...
        log.info("乘用车回笼数据同步 Start--------------");
        while (true) {
            log.info("乘用车回笼数据同步, StartIndex={}", start);
            SelectContractCodeByPageDTO pageInfo = new SelectContractCodeByPageDTO();
            pageInfo.setStart(start);
            pageInfo.setEnd(start + eachQueryNum);
            List<String> contractCodeList = passengerVehicleDataService.selectContractCodeByPage(pageInfo);
        ...
        log.info("乘用车资产转让回笼数据同步 Start--------------");
        while (true) {
            log.info("乘用车资产转让回笼数据同步, StartIndex={}", start);
            com.utfinancing.financehub.etl.passvehat.model.SelectContractCodeByPageDTO pageInfo =
                    new com.utfinancing.financehub.etl.passvehat.model.SelectContractCodeByPageDTO();
            pageInfo.setStart(start);
            pageInfo.setEnd(start + eachQueryNum);
...
//                orElse(null).map(SysDictData::getDictLabel).collect(Collectors.toList());
        ...
        // 按照合同分组
        Map<String, List<SelectRepaymentFromCYCXTDTO>> repaymentMap = repaymentList.stream().
                collect(Collectors.groupingBy(e->e.getContractCode().concat("|").concat(e.getOrgId())));


        // 查詢合同信息
        List<String> contractCodeList = repaymentList.stream().map(SelectRepaymentFromCYCXTDTO::getContractCode).
...
//                toMap(e->e.getContractCode().concat("|").concat(e.getOrgId()), (e)->e, (a, b)->b));
        ...
        List<SelectRepaymentFromCYCXTDTO> newRepaymentList = new ArrayList<>();
        for (String key : repaymentMap.keySet()) {

            List<SelectRepaymentFromCYCXTDTO> repaymentGroupList = repaymentMap.get(key);
...
//            }
    ...
    public void saveReceiveRepayment(List<SelectReceiveRepaymentDTO> receiveRepaymentDTOList) {
        ...
        wrapper.eq(RepaymentPlanExceldataEntity::getSystemCode, SystemEnum.CYCXT.getCode());
