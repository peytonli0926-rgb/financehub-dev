...
-- The real-estate principal rate needs its own amount type because its rate is
-- different from movable assets under the same direct-lease dimensions.
...
SELECT 901,'不动产租赁本金','lease_principal_real_estate','sys_cash_type',NULL,NULL,'N','0',
       'htqz-tax-config-v3',NOW(),'htqz-tax-config-v3',NOW(),'起租不动产本金税率配置专用金额类型'
WHERE NOT EXISTS (
...
SET dict_label='不动产租赁本金',remark='起租不动产本金税率配置专用金额类型',
    update_by='htqz-tax-config-v3',update_time=NOW()
WHERE dict_type='sys_cash_type' AND dict_value='lease_principal_real_estate';
...
CREATE TEMPORARY TABLE tmp_htqz_tax_rate (
 ...
 PRIMARY KEY (business_code,fund_type,lease_type,lease_method)
...
INSERT INTO tmp_htqz_tax_rate VALUES
 ...
 ('ZLYW','lease_principal_receivable','FINANCE_LEASE','DIRECT_LEASE',13.000000),
 ('ZLYW','lease_principal_receivable','FINANCE_LEASE','SALE_AND_LEASEBACK',0.000000),
 ('ZLYW','lease_principal_real_estate','FINANCE_LEASE','DIRECT_LEASE',9.000000),
 ('ZLYW','lease_principal_real_estate','FINANCE_LEASE','SALE_AND_LEASEBACK',0.000000),
 ...
 ('CYC_RETAIL_LEASEBACK','lease_principal_receivable','FINANCE_LEASE','SALE_AND_LEASEBACK',0.000000),
 ('CYC_RETAIL_LEASEBACK','lease_principal_real_estate','FINANCE_LEASE','SALE_AND_LEASEBACK',0.000000),
 ...
 ('JYZL','lease_principal_receivable','OPERATING_LEASE','DIRECT_LEASE',13.000000),
 ('JYZL','lease_principal_receivable','OPERATING_LEASE','SALE_AND_LEASEBACK',13.000000),
 ('JYZL','lease_principal_real_estate','OPERATING_LEASE','DIRECT_LEASE',9.000000),
 ('JYZL','lease_principal_real_estate','OPERATING_LEASE','SALE_AND_LEASEBACK',9.000000),
...
INSERT INTO eg_tax_rate
 (id,business_code,fund_type,enable_flag,enable_date,tax_rate,create_by,create_time,
  update_by,update_time,del_flag,lease_type,lease_sub_type)
