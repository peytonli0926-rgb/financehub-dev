 ...
 * @Description : 合同历史表VO对象
 * @Modified :
 */
@Data
public class ContractHisVO implements Serializable{
    ...
    @ApiModelProperty(value = "组织机构代码/身份证号")
    private String orgIdOrIdentityCard;

    @ApiModelProperty(value = "是否已生成交易结构凭证（0：未生成1：已生成）默认0")
    private String isGenerateVoucher;

    @ApiModelProperty(value = "是否已生成租金计划凭证（0：未生成1：已生成）默认0")
    private String isPlanVoucher;

    @ApiModelProperty(value = "交易结构凭证id todo 需要手工凭证返回")
    private Long voucherId;

    @ApiModelProperty(value = "租金计划凭证id")
