...
public class RetailLeasebackManagementFeeAccrualService {

    public static final String SCENE_CODE = "CYC_ASSET_MANAGEMENT_FEE_ACCRUAL";
    ...
    public List<VoucherDTO> execute(RetailLeasebackManagementFeeAccrualDTO dto) {
        ...
        if (duplicateOrder) throw new ServiceException("请求流水号已存在: " + dto.getOrderId());
        ...
        LocalDateTime monthStart = dto.getBusinessDate().toLocalDate().withDayOfMonth(1).atStartOfDay();
        boolean duplicateMonth = rawTransactionDataService.lambdaQuery()
                .eq(RawTransactionDataEntity::getSceneCode, SCENE_CODE)
                .eq(RawTransactionDataEntity::getContractCode, contract.getContractCode())
                .ge(RawTransactionDataEntity::getBusinessDate, monthStart)
                .lt(RawTransactionDataEntity::getBusinessDate, monthStart.plusMonths(1))
                .eq(RawTransactionDataEntity::getDelFlag, "0").exists();
        if (duplicateMonth) {
            throw new ServiceException("合同" + contract.getContractCode() + "在期间"
                    + dto.getBusinessDate().format(PERIOD_FORMATTER) + "已计提资产管理费");
        }
        ...
        String internalEvent = mapping.getString("managementFeeEventCode");
        if (!SCENE_CODE.equals(sceneCode) || !"JTZCGLF".equals(internalEvent)) {
            throw new ServiceException("计提资产管理费事件未配置值映射: " + dto.getEventCode());
        }

        String accrualPeriod = dto.getBusinessDate().format(PERIOD_FORMATTER);
        ...
        data.put(RuleConstant.FIELD_SCENE_CODE_ORIGINAL, "计提资产管理费");
        data.put("sceneName", "计提资产管理费");
        data.put("eventCode", internalEvent);
        data.put("managementFeeEventCode", internalEvent);
        ...
        data.put("managementFeeAmount", dto.getManagementFeeAmount());
        data.put("accrualPeriod", accrualPeriod);
