...
import { syncCheckExisted, syncSubject } from '@/api/reconciliationReports'
...
const asyncData = async (periodCode) => {
  const { code, msg } = await syncSubject(periodCode)
  if (code === 200) {
...
const permission = computed(() => {
  ...
  return btnPermissions.OutOfPocketPremiums
})
onBeforeMount(() => {
