<template>
  <div>
    <UTable ref="utableRef" @search-form-data="refreshList" :defaultParams="defaultParams" :autoLoad="false"   :options="options" >
...
import { syncCheckExisted, syncSubjectKingdee } from '@/api/reconciliationReports'
...
const asyncData = async (periodCode) => {
  const { code, msg } = await syncSubjectKingdee(periodCode)
  if (code === 200) {
...
const permission = computed(() => {
  ...
  return btnPermissions.subjectKingdee
})
onBeforeMount(() => {
